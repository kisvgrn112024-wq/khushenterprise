    ...
    <div className="max-w-6xl mx-auto pb-12 relative text-left">
      ...
      {isQuoteModalOpen && (
        <div className="fixed inset-0 bg-theme/80 flex items-center justify-center z-50">
          <div className="bg-theme border border-theme/10 rounded-xl p-6 w-[550px] text-left">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-xl font-bold text-theme">Generative Quote Builder</h2>
                <div className="text-xs text-theme mt-1">Inquiry {activeQuoteId}</div>
              </div>
              <button onClick={() => setIsQuoteModalOpen(false)} className="text-theme hover:text-theme"><X size={20} /></button>
            </div>
            
            <div className="space-y-4">
              <div className="bg-theme p-4 rounded border border-theme/5">
                <div className="text-xs text-theme mb-2">Automated Margin Calculation</div>
                <div className="flex justify-between items-center mb-2">
                  <span className="text-sm text-theme">Base Total (Wholesale)</span>
                  <span className="text-sm text-theme font-mono">₹45,000.00</span>
                </div>
                <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex justify-between items-center border-t border-theme/5 pt-2 mt-2">
                  <span className="text-sm font-bold text-theme">Proposed Final Quote</span>
                  <span className="text-lg font-bold text-brand-yellow font-mono">₹40,500.00</span>
                </div>
              </div>

              <div>
                <label className="block text-[11px] font-bold text-theme uppercase tracking-widest mb-2">Custom Adjustments (%)</label>
                <input type="number" defaultValue={10} className="w-full bg-theme border border-theme/5 rounded px-4 py-3 text-theme text-sm outline-none focus:border-theme" />
              </div>
              
              <div>
                <label className="block text-[11px] font-bold text-theme uppercase tracking-widest mb-2">Additional Terms (Optional)</label>
                <textarea rows={3} placeholder="e.g. Free shipping and delivery within 5 days..." className="w-full bg-theme border border-theme/5 rounded px-4 py-3 text-theme text-sm outline-none focus:border-theme resize-none"></textarea>
              </div>
            </div>
            
            <div className="flex justify-end gap-3 mt-8">
              <button onClick={() => setIsQuoteModalOpen(false)} className="px-4 py-2 text-sm font-bold text-theme hover:text-theme transition-colors">Cancel</button>
              <button onClick={() => {
                setIsQuoteModalOpen(false);
                startDownload(`Quote_${activeQuoteId.replace('#', '')}.pdf`, "0.8 MB", "PDF Document");
              }} className="px-6 py-2 bg-theme hover:bg-theme text-theme font-bold text-sm rounded transition-colors flex items-center gap-2">
                Generate & Export PDF
              </button>
            </div>
          </div>
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-theme mb-1">Bulk Order Management</h1>
          <p className="text-theme text-sm">Overseeing high-volume inquiries, GST inputs, and institutional procurement.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-theme border border-theme/5 rounded-lg p-5">
          ...
          <div className="text-[10px] font-bold text-theme uppercase tracking-widest mb-1">Total Inquiries</div>
          <div className="text-3xl font-bold text-theme">{combinedList.length}</div>
        </div>
        <div className="bg-theme border border-theme/5 rounded-lg p-5">
          <div className="flex items-center justify-between mb-4">
          ...
          <div className="text-[10px] font-bold text-theme uppercase tracking-widest mb-1">Pending Processing</div>
        ...
        <div className="bg-theme border border-theme/5 rounded-lg p-5">
          ...
          <div className="text-[10px] font-bold text-theme uppercase tracking-widest mb-1">Shipped Inquiries</div>
          <div className="text-3xl font-bold text-theme">
        ...
        <div className="bg-theme border border-theme/5 rounded-lg p-5">
          <div className="flex items-center justify-between mb-4">
            <Target size={16} className="text-theme" />
            <span className="text-[10px] font-bold text-theme tracking-wider">Q4 Sourcing</span>
          </div>
          <div className="text-[10px] font-bold text-theme uppercase tracking-widest mb-1">Tax-deductible Ledger</div>
          <div className="text-3xl font-bold text-theme">18% ITC</div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-4 items-end justify-between mb-6">
        <div className="flex items-center gap-4">
          <div>
            <label className="block text-[10px] font-bold text-theme uppercase tracking-widest mb-2">Filter Status</label>
            <div className="flex bg-theme border border-theme/5 rounded p-1">
              <button 
                onClick={() => setSelectedStatus("All")}
                className={`px-4 py-1 rounded text-xs font-bold transition-colors ${
                  selectedStatus === "All" ? "bg-theme text-[#8bceff] border border-theme/20" : "text-theme hover:text-theme"
                }`}
              >
                All
              </button>
              <button 
                ...
                className={`px-4 py-1 rounded text-xs font-bold transition-colors ${
                  selectedStatus === "Pending" ? "bg-theme text-[#8bceff] border border-theme/20" : "text-theme hover:text-theme"
                }`}
              >
                Pending
              </button>
              <button 
                ...
                className={`px-4 py-1 rounded text-xs font-bold transition-colors ${
                  selectedStatus === "Shipped" ? "bg-theme text-[#8bceff] border border-theme/20" : "text-theme hover:text-theme"
                }`}
              >
                Shipped
              </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => startDownload("Bulk_Inquiries_Export.csv", "3.4 MB", "CSV File")} className="bg-theme border border-theme/5 hover:bg-theme/10 text-theme p-2.5 rounded transition-colors">
            <Download size={16} />
          </button>
        </div>
      </div>
        ...
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-white/5">
            {filteredList.length === 0 ? (
              <tr>
                <td colSpan={7} className="p-10 text-center text-theme font-light">
              ...
              filteredList.map((inq, idx) => (
                <tr key={idx} className="hover:bg-theme/[0.02] transition-colors">
                  ...
                  <td className="p-5">
                    <div className="text-theme font-bold mb-1 uppercase tracking-wider">{inq.institution}</div>
                    <div className="text-xs text-theme">{inq.officer}</div>
                  </td>
                  <td className="p-5">
                    <div className="flex flex-wrap gap-1.5 max-w-xs">
                      {inq.items.map((item: string, i: number) => (
                        <span key={i} className="px-2 py-1 bg-theme border border-theme/5 rounded text-[10px] text-theme">{item}</span>
                      ))}
                    </div>
                  </td>
                  <td className="p-5 font-bold text-theme font-mono">{inq.value}</td>
                  <td className="p-5 text-xs text-theme max-w-sm">
                  ...
                  <td className="p-5">
                    <select 
                      ...
                      className="bg-theme border border-theme/5 text-theme text-xs py-1.5 px-2 rounded outline-none cursor-pointer hover:bg-theme/5 transition-colors appearance-none min-w-[120px]"
                    >
                    ...
                    <div className="flex items-center justify-end gap-3">
                      <button 
                        ...
                        className={`px-4 py-1.5 rounded text-[10px] font-bold uppercase tracking-wider transition-colors ${
                          inq.action === 'GENERATE QUOTE' ? 'bg-theme hover:bg-theme text-theme' :
                          inq.action === 'DOWNLOAD PO' ? 'bg-theme/10 hover:bg-theme/20 text-theme' :
                          'bg-theme/5 border border-theme/10 hover:bg-theme/10 text-theme'
                        }`}
                      >
                        {inq.action}
                      </button>
