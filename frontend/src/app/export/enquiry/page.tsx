...
export default function ExportEnquiryPage() {
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center px-4">
        ...
        >
          ...
          <h2 className="text-2xl font-black text-white mb-3">Inquiry Submitted to Anvil</h2>
          <p className="text-gray-400 text-sm mb-2">
            Your export inquiry has been securely routed to the KE management dashboard.
          </p>
          <p className="text-gray-600 text-xs mb-8 font-mono">Secure transmissions active.</p>

          <div className="bg-[#111] border border-white/5 rounded-xl p-5 text-left mb-6 space-y-2">
            ...
            <div className="flex justify-between text-xs">
              ...
              <span className="text-white font-medium">{form.destinationCountry}</span>
            </div>
            <div className="flex justify-between text-xs">
              <span className="text-gray-500">Logistics</span>
              <span className="text-white font-medium">{form.logisticsVector}</span>
            </div>
            <div className="flex justify-between text-xs">
          ...
          <div className="flex gap-3 justify-center">
            <Link href="/export">
              <button className="px-6 py-2.5 border border-white/10 rounded text-xs font-bold text-gray-400 hover:text-white hover:border-white/25 transition-all uppercase tracking-wider">
                Export Hub
              </button>
            </Link>
            <button
              onClick={() => { setSubmitted(false); setForm(INITIAL); }}
              className="px-6 py-2.5 bg-brand-yellow hover:bg-yellow-400 text-black rounded text-xs font-black uppercase tracking-wider transition-colors"
            >
              New Enquiry
            </button>
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white flex">
      {/* ── Sidebar ── */}
      <aside className="hidden md:flex flex-col w-56 bg-[#0d0d0d] border-r border-white/5 min-h-screen sticky top-[73px] h-screen pt-6 pb-10 px-3">
        ...
        <div className="mt-6 px-3 space-y-2">
          ...
          <Link href="/contact-us" className="flex items-center gap-2 text-gray-600 hover:text-gray-400 text-[10px] transition-colors">
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#111] border border-white/10 rounded px-4 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-brand-yellow/40 transition-colors"
                />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#111] border border-white/10 rounded px-4 py-3 text-sm text-white outline-none focus:border-brand-yellow/40 transition-colors appearance-none"
                >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-[#111] border border-white/10 rounded px-4 py-3 text-sm text-white placeholder-gray-600 outline-none focus:border-brand-yellow/40 transition-colors resize-none"
              />
            </div>
            ...
            <motion.button
              ...
              className="flex items-center gap-3 bg-brand-yellow hover:bg-yellow-400 disabled:opacity-60 disabled:cursor-not-allowed text-black px-8 py-3.5 rounded text-xs font-black uppercase tracking-wider transition-colors"
            >
              {loading ? (
                <>
        ...
        <div className="px-6 py-4 border-t border-white/5 text-[9px] text-gray-700 font-mono flex flex-wrap gap-4">
          ...
          <Link href="/contact-us" className="hover:text-gray-500 transition-colors">Manager Support</Link>
