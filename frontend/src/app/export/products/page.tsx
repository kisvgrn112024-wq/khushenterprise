    ...
    <div className="min-h-screen bg-theme text-theme flex">
          ...
          <Link href="/export/enquiry" className="mx-2 mt-4">
            <div className="bg-brand-yellow/10 border border-brand-yellow/30 rounded-lg p-3 hover:bg-brand-yellow/15 transition-colors">
              ...
              <div className="text-theme font-bold text-lg">
                {enquiry.length}{" "}
                <span className="text-theme text-xs font-normal">items</span>
              </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="border-b border-theme/5 bg-theme px-6 py-5">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div>
              ...
              <h1 className="text-2xl md:text-3xl font-black text-theme">Export Products</h1>
        ...
        <div className="px-6 py-4 border-b border-theme/5 bg-theme flex flex-col sm:flex-row flex-wrap gap-3 items-start sm:items-center">
          <div className="flex items-center gap-2 text-theme text-[10px] font-bold uppercase tracking-widest flex-shrink-0">
          ...
          <div className="relative ml-auto flex-shrink-0">
            <select
              ...
              className="bg-theme border border-theme/10 rounded px-3 py-2 text-[10px] text-theme outline-none appearance-none pr-7 focus:border-theme/25"
            >
              {LOGISTICS.map((l) => <option key={l}>{l}</option>)}
            </select>
            <ChevronDown size={11} className="absolute right-2 top-1/2 -translate-y-1/2 text-theme pointer-events-none" />
          </div>
        </div>
        ...
        <div className="flex-1 px-6 py-6">
          {loading ? (
            ...
            <div className="text-center py-20 text-theme">
              ...
              <p className="text-sm font-bold text-theme mb-1">No products found</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
              {filtered.map((product, i) => (
                <motion.div
                  ...
                  className="group bg-theme border border-theme/5 rounded-xl overflow-hidden hover:border-brand-yellow/30 transition-all hover:shadow-[0_0_20px_rgba(252,211,77,0.06)] flex flex-col"
                >
                  {/* Image */}
                  <div className="relative h-44 bg-theme overflow-hidden">
                    {product.images?.[0] ? (
                      <img
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Package2 size={36} className="text-theme" />
                      </div>
                    )}
                    <div className="absolute top-2 left-2 flex gap-1 flex-wrap">
                      <span className="bg-brand-yellow/90 text-theme text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-wider">
                        Export Ready
                      </span>
                      {product.tag && (
                        <span className="bg-theme/70 text-brand-yellow text-[9px] font-bold px-2 py-0.5 rounded uppercase border border-brand-yellow/20">
                          {product.tag}
                        </span>
                      )}
                    </div>
                    {/* Logistics badge */}
                    <div className="absolute bottom-2 right-2 bg-theme/60 border border-theme/10 backdrop-blur-sm rounded px-2 py-1 flex items-center gap-1">
                      {logistics === "Air Freight"
                        ? <Plane size={10} className="text-brand-yellow" />
                        : <Ship size={10} className="text-electric-blue" />}
                      <span className="text-[8px] text-theme font-bold">{logistics}</span>
                    </div>
                  </div>

                  {/* Info */}
                  <div className="p-4 flex-1 flex flex-col">
                    <div className="text-theme text-[9px] font-bold uppercase tracking-widest mb-1">
                      {product.category || "General"}
                    </div>
                    <h3 className="text-theme font-bold text-sm mb-1 line-clamp-2 group-hover:text-brand-yellow transition-colors leading-snug">
                      {product.title}
                    </h3>
                    {product.sku && (
                      <div className="text-theme text-[9px] font-mono mb-2">SKU: {product.sku}</div>
                    )}

                    <div className="mt-auto pt-3 border-t border-theme/5">
                      <div className="flex items-center justify-between mb-3">
                        <div>
                          <div className="text-[9px] text-theme uppercase font-bold">Export Price</div>
                          <div className="text-brand-yellow font-black text-base leading-none mt-0.5">
                            ₹{product.price?.toLocaleString("en-IN") ?? "—"}
                          </div>
                          {product.originalPrice && (
                            <div className="text-theme text-[9px] line-through mt-0.5">
                        ...
                        {product.stock !== undefined && (
                          <div className="text-right">
                            <div className="text-[9px] text-theme uppercase font-bold">Stock</div>
                            <div className={`font-bold text-sm ${product.stock === 0 ? "text-red-400" : product.stock < 10 ? "text-yellow-400" : "text-theme"}`}>
                              {product.stock}
                            </div>
                          </div>
                        )}
                      ...
                      <button
                        ...
                        className={`w-full py-2 rounded text-[10px] font-black uppercase tracking-wider transition-all border ${
                          flashId === product.id
                            ...
                            ? "bg-theme/5 border-theme/10 text-theme cursor-default"
                            : "bg-brand-yellow hover:bg-yellow-400 border-transparent text-theme hover:scale-[1.01]"
                        }`}
                      >
                        {flashId === product.id
                          ? "✓ Added to Enquiry"
          ...
          {!loading && filtered.length > 0 && (
            <motion.div
              ...
              className="mt-8 bg-theme border border-theme/5 rounded-xl p-5"
            >
              <div className="text-brand-yellow text-[10px] font-black uppercase tracking-widest mb-2">
                Logistics Notice
              </div>
              <p className="text-theme text-xs leading-relaxed">
                All prices listed are FOR (Free on Road) India prices. Freight forwarding, customs duty, insurance costs,
                and local destination taxes are <strong className="text-theme">NOT</strong> included. Est. Air Freight:{" "}
                <strong className="text-theme">08–35 days</strong> · Sea Freight:{" "}
                <strong className="text-theme">24+ days</strong> to EU/GCC region.
              </p>
            </motion.div>
          )}
        </div>
