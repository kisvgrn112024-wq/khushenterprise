...
export default function CheckoutPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050b14] text-slate-300">
      {/* Simplified Header */}
      <header className="bg-midnight-navy border-b border-white/10 py-4 px-8 sticky top-0 z-50">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2">
            <Image src="/logo.png" alt="Khush Enterprises" width={32} height={32} />
            <span className="font-bold text-white tracking-widest text-lg">KHUSH ENTERPRISES</span>
          </Link>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          {/* Step 1: Shipping */}
          <div className={`glass-dark rounded-xl border ${step === 1 ? 'border-neon-cyan/50 box-glow' : 'border-white/5 opacity-70'} overflow-hidden`}>
              ...
              <form onSubmit={handleNext} className="p-6 space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input required type="text" className="w-full bg-black/20 border border-slate-700 rounded px-3 py-2 text-white outline-none focus:border-neon-cyan" />
                  </div>
                  <div>
                    <label className="block text-sm text-slate-400 mb-1">Phone Number</label>
                    <input required type="tel" className="w-full bg-black/20 border border-slate-700 rounded px-3 py-2 text-white outline-none focus:border-neon-cyan" />
                  </div>
                </div>
                <div>
                ...
                <button type="submit" className="bg-electric-blue hover:bg-blue-600 text-white font-medium py-3 px-8 rounded mt-4 transition-colors">
          ...
          <div className={`glass-dark rounded-xl border ${step === 2 ? 'border-neon-cyan/50 box-glow' : 'border-white/5 opacity-70'} overflow-hidden`}>
            <div className="p-4 bg-white/5 border-b border-white/5 flex items-center justify-between">
              <h2 className="text-lg font-bold text-white flex items-center gap-2">
            ...
            {step === 2 && (
              <div className="p-6">
                <div className="space-y-4 mb-6 max-h-64 overflow-y-auto pr-2">
                  {cart.map(item => (
                    <div key={item.id} className="flex justify-between border-b border-white/5 pb-4">
                      <div>
                ...
                <button onClick={(e) => handleNext(e as any)} className="bg-electric-blue hover:bg-blue-600 text-white font-medium py-3 px-8 rounded transition-colors">
                  Proceed to Payment
                </button>
              </div>
            )}
          ...
          {/* Step 3: Payment */}
          <div className={`glass-dark rounded-xl border ${step === 3 ? 'border-neon-cyan/50 box-glow' : 'border-white/5 opacity-70'} overflow-hidden`}>
            ...
            {step === 3 && (
              <div className="p-6 space-y-4">
                ...
                {/* Prepaid UPI / QR */}
                <div className={`border rounded-lg transition-colors overflow-hidden ${paymentMethod === 'upi' ? 'border-neon-cyan/50 bg-neon-cyan/5' : 'border-white/10 hover:border-white/30'}`}>
                  <label className="flex items-center justify-between p-4 cursor-pointer" onClick={() => setPaymentMethod('upi')}>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-white font-medium flex items-center gap-2"><QrCode size={18}/> Prepaid UPI / QR Code</span>
                    </div>
                    ...
                    <div className="p-4 pt-0 border-t border-white/5 mt-2 bg-black/20">
                      <div className="flex flex-col md:flex-row gap-6 items-center md:items-start p-4">
                        ...
                        <div className="flex-1 space-y-3 w-full">
                          ...
                          <div className="bg-[#111111] border border-white/10 p-3 rounded flex justify-between items-center">
                            <div>
                              <div className="text-[10px] text-gray-500 font-bold tracking-widest uppercase">UPI ID</div>
                              <div className="text-white font-mono text-sm">immalhotra57-2@okhdfcbank</div>
                            </div>
                            <button onClick={() => navigator.clipboard.writeText('immalhotra57-2@okhdfcbank')} className="text-xs text-neon-cyan hover:text-white transition-colors">Copy</button>
                          </div>
                          
                          <div className="mt-4 border border-dashed border-white/20 rounded p-4 text-center hover:border-neon-cyan/50 transition-colors cursor-pointer bg-white/5" onClick={() => {
                            // Simulate upload
                ...
                {/* Cash on Delivery */}
                <div className={`border rounded-lg transition-colors ${paymentMethod === 'cod' ? 'border-neon-cyan/50 bg-neon-cyan/5' : 'border-white/10 hover:border-white/30'}`}>
                  <label className="flex items-center justify-between p-4 cursor-pointer" onClick={() => setPaymentMethod('cod')}>
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-white font-medium flex items-center gap-2"><Truck size={18}/> Cash on Delivery (COD)</span>
                    </div>
                  </label>
                  {paymentMethod === 'cod' && (
                ...
                <label className="flex items-center justify-between p-4 border border-white/10 hover:border-white/30 rounded-lg cursor-pointer transition-colors" onClick={() => setPaymentMethod('card')}>
                  <div className="flex items-center gap-3">
                    <input type="radio" name="payment" checked={paymentMethod === 'card'} onChange={() => setPaymentMethod('card')} className="accent-neon-cyan" />
                    <span className="text-white font-medium flex items-center gap-2"><CreditCard size={18}/> Credit / Debit Card</span>
                  </div>
                </label>
                ...
                <button 
                  ...
                  className={`w-full font-bold py-4 rounded-lg mt-6 text-lg transition-colors shadow-[0_0_15px_rgba(0,255,255,0.4)] ${paymentMethod === 'upi' && !screenshotUploaded ? 'bg-gray-600 text-gray-400 cursor-not-allowed shadow-none' : 'bg-neon-cyan hover:bg-white text-midnight-navy'}`}
                >
        ...
        <div className="lg:col-span-1">
          <div className="glass-dark rounded-xl border border-white/5 p-6 sticky top-24">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-500 bg-white/5 p-3 rounded">
