...
export default function Header() {
  ...
  if (pathname?.startsWith("/admin-secret-khush")) {
    return null;
  }
        ...
        <div className="bg-gradient-to-r from-blue-900 to-indigo-950 border-b border-blue-500/20 text-[#8bceff] px-4 py-2 text-[10px] font-bold flex justify-between items-center relative z-50">
          <div className="flex items-center gap-1.5 mx-auto">
            ...
            <span className="truncate max-w-[250px] sm:max-w-none">
              ðŸšš Dispatch Alert: Order {dispatchedOrder.id} dispatched via Blue Dart Express (Tracking ID: {dispatchedOrder.tracking})
            </span>
      ...
      <div className="container mx-auto px-4 lg:px-8 py-4">
                ...
                {searchResults.map(product => (
                  <Link key={product.id} href={`/products?q=${encodeURIComponent(product.title)}`} className="flex items-center gap-3 p-3 hover:bg-theme border-b border-theme/5 last:border-0">
                    ...
                    <div>
                      ...
                      <div className="text-theme text-xs font-semibold">â‚¹{product.price.toLocaleString("en-IN")}</div>
                    </div>
                  </Link>
                ))}
              </div>
        ...
        <nav className={`${viewMode === "mobile" ? "hidden" : "hidden md:flex md:justify-center md:overflow-visible whitespace-nowrap pb-2 md:pb-0 hide-scrollbar"} items-center gap-5 sm:gap-8 text-[9px] sm:text-xs font-semibold text-theme uppercase tracking-wider mt-1 px-1`}>
          ...
          >
            ...
            >
              Bulk Orders
              <span className="text-[8px] transition-transform group-hover:rotate-180">â–¼</span>
            </Link>
      ...
      {isMobileMenuOpen && (
        <div className="fixed inset-0 z-[1000] flex justify-end">
          ...
          <div className="relative w-64 max-w-xs bg-theme border-l border-theme/5 h-full p-6 flex flex-col z-50 shadow-2xl overflow-y-auto">
            ...
            {/* â”€â”€ Smart Search Bar (full featured, inside drawer) â”€â”€ */}
            <div className="mb-4">
              <form
              ...
              >
                <input
                  type="text"
                  placeholder={isListening ? "Listeningâ€¦" : "Search equipmentâ€¦"}
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  onFocus={() => setIsSearchFocused(true)}
                  onBlur={() => setTimeout(() => setIsSearchFocused(false), 200)}
                  ...
                  {searchResults.map(product => (
                    ...
                    >
                      ...
                      <div>
                        ...
                        <div className="text-theme text-[10px] font-semibold">â‚¹{product.price.toLocaleString("en-IN")}</div>
                      </div>
                    </Link>
                  ))}
                </div>
            ...
            <div className="flex flex-col gap-3 pb-4 mb-4 border-b border-theme/5">
              {/* Theme Switcher â€” prominently displayed */}
            ...
            <nav className="flex flex-col gap-0 text-xs font-semibold uppercase tracking-wider text-theme">
              ...
              <div className="flex flex-col">
                ...
                >
                  ...
                  <span className={`text-[10px] transform transition-transform ${isMobileBulkOpen ? "rotate-180" : ""}`}>â–¼</span>
                </button>
                ...
                {isMobileBulkOpen && (
                  <div className="pl-4 py-2 flex flex-col gap-3 text-[11px] normal-case text-theme border-l border-theme/5 ml-2 mt-1">
                    ...
                    <Link href="/bulk-orders?sector=b2b" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-theme">ðŸ’¼ B2B Business</Link>
                    <Link href="/bulk-orders?sector=institutional&sub=schools" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-theme">ðŸ« School Lab Solutions</Link>
                    <Link href="/bulk-orders?sector=institutional&sub=colleges" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-theme">ðŸŽ“ College Packages</Link>
                    <Link href="/bulk-orders?sector=institutional&sub=research" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-theme">ðŸ”¬ Advanced Research Labs</Link>
                    <Link href="/bulk-orders?sector=commercial" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-theme">ðŸ­ Commercial Labs</Link>
                  </div>
                )}
              </div>
