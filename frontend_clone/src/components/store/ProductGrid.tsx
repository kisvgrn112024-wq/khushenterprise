...
export default function ProductGrid() {
  ...
  return (
    <div className="w-full bg-theme py-16">
      <div className="container mx-auto px-4 lg:px-8">
        <div className="flex flex-col mb-10 pb-4 border-b border-theme/10">
          <h2 className="text-3xl md:text-4xl font-bold text-theme mb-2">Featured Equipment</h2>
          <p className="text-theme text-sm md:text-base">Precision engineered solutions most trusted by our institutional partners.</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {products.map((product, idx) => {
            ...
            return (
              <div key={product.id} className="bg-theme border border-theme/10 flex flex-col group relative p-4 hover:border-electric-blue/50 transition-colors">
                {/* Product Image Area */}
                <Link href={`/products/${product.id}`} className="bg-theme p-6 mb-4 flex items-center justify-center h-48 relative cursor-pointer overflow-hidden transition-colors rounded">
                  {/* Badges */}
                  ...
                  <IconComponent size={60} className="text-theme transform group-hover:scale-105 transition-transform duration-500 hidden" strokeWidth={1} />
                </Link>
                ...
                <div className="flex flex-col flex-1 relative">
                  <div className="flex justify-between items-center mb-1">
                    ...
                    <div className="flex items-center gap-1 bg-theme px-1.5 py-0.5 rounded border border-theme/5">
                      ...
                      <span className="text-[9px] font-black text-theme tracking-widest">KE</span>
                    </div>
                  </div>
                  
                  <Link href={`/products/${product.id}`} className="text-theme text-lg font-bold line-clamp-2 hover:text-theme transition-colors mb-6">
                    {displayTitle}
                  </Link>
                  
                  <div className="flex items-center justify-between mt-auto">
                    <div className="flex items-baseline gap-2">
                      <span className="text-xl font-bold text-theme">₹{product.price.toLocaleString()}</span>
                    </div>

                    <button 
                      onClick={() => addToCart(product)}
                      className="w-10 h-10 bg-brand-yellow text-theme flex items-center justify-center rounded hover:bg-yellow-500 transition-colors transform hover:scale-105 active:scale-95 shadow-[0_0_10px_rgba(252,211,77,0.2)]"
